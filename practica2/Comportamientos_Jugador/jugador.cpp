...
Action ComportamientoJugador::think(Sensores sensores)
{
	...
	if (!hayPlan)
	{
		...
		if (sensores.nivel == 0)
			...
			plan = nivel_2_3(c_state, goal, mapaResultado, 2);
...
unsigned short ComportamientoJugador::coste_accion_total(const Action a, const estado &st, const vector<vector<unsigned char> > &mapa) const
{
	if (a == act_CLB_WALK or a == act_CLB_TURN_SR or a == act_CLB_STOP)
		return coste_accion(a, st, mapa);
	else
		return coste_accion(a, st, mapa) + coste_accion(st.ultima_orden_colaborador, st, mapa);
}

unsigned short ComportamientoJugador::coste_accion(const Action a, const estado &st, const vector<vector<unsigned char>> &mapa) const
{
...
void ComportamientoJugador::generar_hijo(const nodo &padre, const Action a, nodo &hijo, const vector<vector<unsigned char>> &mapa, const unsigned char nivel) const
{
	...
	if (nivel > 1)
	{
		...
		else if (mapa[hijo.st.jugador.f][hijo.st.jugador.c] == 'D')
		{
		...
		if (nivel > 2)
		{
			...
			else if (mapa[hijo.st.colaborador.f][hijo.st.colaborador.c] == 'D')
			{
...
estado ComportamientoJugador::apply(const Action &a, const estado &st, const vector<vector<unsigned char>> &mapa) const
{
	...
	{
	case actWALK:
		if (casilla_transitable(st_sig, mapa, false))
			st_result = st_sig;
		break;
	...
	case actRUN:
		if (casilla_transitable(st_sig, mapa, false) && casilla_transitable(st_sig2, mapa, false))
			st_result = st_sig2;
		break;
...
queue<Action> ComportamientoJugador::nivel_2_3(const estado &inicio, const ubicacion &final, const vector<vector<unsigned char>> &mapa, const unsigned char nivel)
{
