...
int main(int argc, char **argv)
{
  ...
  else
  {
    ...
    ifile.close();
    // Reviso si es una posicion valida del jugador
    if (atoi(argv[4]) != -1)
    {
    ...
    //cerr << "Umbral porcentaje bateria: " << stof(argv[argc-3]) << endl;
    monitor.get_entidad(0)->umbral_ciclos_recarga = stof(argv[argc-2]);
